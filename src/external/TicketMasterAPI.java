...
public class TicketMasterAPI implements ExternalAPI {
	...
	private static final String DEFAULT_TERM = "ticket";//used when no restriction
	...
	public List<Item> search(double lat, double lon, String term) {
		...
		try {
			...
			JSONObject embeded = (JSONObject) responseJson.get("_embedded");
			JSONArray events = (JSONArray) embeded.get("events");
...
/*  public static void main(String[] args) {
  		List<Item> itemList = ExternalAPIFactory.getExternalAPI(ExternalAPINames.TICKETMASTER_API).search(37.38, -122.08, "Music");
		try {
 ...
 /*test	
	public static void main(String[] args) {
		...
		JSONArray events = tMasterAPI.search(37.38, -122.08, null);
		try {
			for (int i = 0; i < events.length(); i++) {
			    JSONObject event = events.getJSONObject(i);
