(function() {
	...
	function init() {
		...
		$('newuser-btn').addEventListener('click', toRegister);
	 ...
	 * */
	function register(){
		var username = $('user_id').value;
		var password1 = $('psw').value;
		var firstname = $('firstname').value;
		var lastname = $('lastname').value;
		var password = md5(username + md5(password1));
		
		var hint = validate(username, password1, firstname, lastname);
		if(hint != null) {
			...
			function(res) {
				...
				if(result.status === 'OK') {
					...
					var hint2 = 'This username has been registered, please change one :).';
					showRegisterError(hint2)
				}
			}
			);
		}
	...
	function validate(username, password, firstname, lastname) {
		...
		if(username=="" || password==""  || firstname==""  || lastname=="" ) {
			 hint = 'One or more fields are missing. Please fill them all. \n';
			 return hint;
		}else {
			return null;
		}
	 ...
	 * Show register Error
	 * */
	function showRegisterError(hint) {
		$('register-error').innerHTML = hint;
	}
